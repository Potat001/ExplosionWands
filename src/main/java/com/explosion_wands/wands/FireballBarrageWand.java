...
public class FireballBarrageWand {
    ...
    public static InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        double xDir = 0;
        //Increases the downwards speed since it causes problems when the speed is too low
        double yDir = -5;
        double zDir = 0;
        int degrees = 90;
        if(level instanceof ServerLevel serverLevel) {
            ...
            /*
            EntityHitResult entityHitResult = ProjectileUtil.getEntityHitResult(Objects.requireNonNull(level.getEntity(1)),
                    playerEyeStart,
                    playerEyeEnd,
